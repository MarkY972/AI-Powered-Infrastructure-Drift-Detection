...
def main():
    ...
    drift_detected_output = "false"
    ai_parsed_recommendation_output = "NONE"
    error_occurred = False
    error_subject = "Drift Detection Error"
    error_message = ""
        ...
        if drift_info:
            ...
            if drift_info["has_drift"]:
                ...
                if ai_client:
                    ...
                    if ai_summary:
                        logging.info("AI Analysis Summary (Raw):")
                        logging.info(ai_summary)
                        ai_parsed_recommendation_output = parse_ai_recommendation(ai_summary)
                        logging.info(f"AI Parsed Recommendation: {ai_parsed_recommendation_output}")
                    else:
                        logging.warning("AI analysis failed or returned no summary.")
                        ai_parsed_recommendation_output = "AI_ANALYSIS_FAILED"
                else:
                    logging.warning("OpenAI client not available. Skipping AI analysis.")
                    ai_parsed_recommendation_output = "AI_CLIENT_UNAVAILABLE"

                # Send SNS notification about the drift
                sns_client = get_sns_client()
                if sns_client and SNS_TOPIC_ARN:
                    subject = f"[{ai_parsed_recommendation_output}] EKS Infrastructure Drift Detected: {drift_info.get('resource_changes_count',0)} resources, {drift_info.get('output_changes_count',0)} outputs"
                    message_body = f"Drift Summary: {drift_info.get('summary')}\n\n"
                    message_body += f"Parsed AI Recommendation: {ai_parsed_recommendation_output}\n\n"
                    message_body += f"Drift Details (JSON):\n{json.dumps(drift_info, indent=2)}\n\n"
                    if ai_summary:
                        message_body += f"Full AI Analysis:\n{ai_summary}"
                    else:
                ...
                ai_parsed_recommendation_output = "NO_DRIFT"
            ...
            error_occurred = True
            error_subject = "Drift Detection Error: Parse Failure"
            error_message = "Failed to parse the Terraform plan. Check workflow logs for details."
            ai_parsed_recommendation_output = "ERROR_PLAN_PARSE"
    else:
        logging.error("Failed to generate Terraform plan.")
        error_occurred = True
        error_subject = "Drift Detection Error: Plan Generation Failure"
        error_message = "Failed to generate the Terraform plan. Check workflow logs for details."
        ai_parsed_recommendation_output = "ERROR_PLAN_GENERATE"

    if error_occurred:
        sns_client = get_sns_client()
        if sns_client and SNS_TOPIC_ARN:
            publish_sns_message(sns_client, SNS_TOPIC_ARN, error_subject, error_message)

    # Set GitHub Actions outputs
    # Ensure these are always printed, even if values are defaults like "false" or "NONE"
    print(f"::set-output name=drift_detected::{drift_detected_output}")
    print(f"::set-output name=ai_parsed_recommendation::{ai_parsed_recommendation_output}")
    logging.info(f"Setting GitHub Action output: drift_detected={drift_detected_output}")
    logging.info(f"Setting GitHub Action output: ai_parsed_recommendation={ai_parsed_recommendation_output}")
...
def parse_ai_recommendation(ai_text_response):
    """
    Parses the AI's textual response to extract a programmable recommendation.

    Args:
        ai_text_response (str): The raw text response from the AI.

    Returns:
        str: A standardized action string (e.g., "APPLY_SAFE", "MANUAL_REVIEW", "UNKNOWN").
    """
    if not ai_text_response:
        return "UNKNOWN"

    response_lower = ai_text_response.lower()

    # Look for keywords indicating safety or need for review
    # This is a simple keyword search; more sophisticated parsing could be done (e.g., regex, NLP)
    if "recommendation: safe to apply" in response_lower or "safe to apply automatically" in response_lower :
        return "APPLY_SAFE"
    elif "recommendation: requires manual review" in response_lower or "suggest manual review" in response_lower or "not safe to apply" in response_lower:
        return "MANUAL_REVIEW"

    # Fallback if specific phrases aren't found
    if "safe to apply" in response_lower: # More generic check
        return "APPLY_SAFE"
    if "manual review" in response_lower or "requires review" in response_lower:
        return "MANUAL_REVIEW"

    logging.warning(f"Could not determine a clear programmable action from AI response. Defaulting to UNKNOWN. AI Response: {ai_text_response[:500]}...")
    return "UNKNOWN"
