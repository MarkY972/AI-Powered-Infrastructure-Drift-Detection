...
*   **Clarity and Parsability**: Your analysis and recommendations should be clear, concise, and easily understandable by a DevOps engineer.
    *   The script `scripts/drift_detector.py` (specifically the `analyze_drift_with_ai` function) sends you a prompt that requests a specific output format.
    *   **Crucially, ensure your response includes a line like `Recommendation: Safe to apply` or `Recommendation: Requires manual review`**. The `parse_ai_recommendation` function in the script looks for these key phrases (case-insensitive) to determine a programmable action. Other variations like "safe to apply automatically" or "suggest manual review" are also searched for. Adhering to this helps the system categorize your response for automated actions (like creating GitHub issues).
